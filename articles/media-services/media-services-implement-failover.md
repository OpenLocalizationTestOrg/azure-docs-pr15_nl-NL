<properties 
    pageTitle="Failover scenario streaming implementeren | Microsoft Azure" 
    description="Dit onderwerp wordt uitgelegd hoe u failover streaming scenario implementeert." 
...
#<a name="implementing-failover-streaming-scenario"></a>Failover scenario streaming implementeren

Deze procedure laat zien hoe inhoud (BLOB's) kopiëren van het ene activum naar een andere redundantie voor het op aanvraag streaming verwerken. Dit scenario is handig voor klanten die hun CDN foutherstel tussen twee datacenters in het geval van een storing in een van onze datacenters instellen.
In dit scenario gebruikmaakt van Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST API en Azure opslag SDK om te laten zien van de volgende taken uitvoeren.

1. Stel een Media Services-account in "Data Center A".
1. Een mezzanine-bestand uploaden naar een activum bron.
1. Coderen van de activa in als u meerdere bit tarief MP4-bestanden. 
1. Maak een alleen-lezen SA's locator voor de bron-activa alleen toegang heeft tot de container in het opslag-account dat is gekoppeld aan de activa bron.
1. De containernaam van de van de activa bron krijgen van de alleen-lezen SA's locator in de vorige stap hebt gemaakt. Deze gegevens zijn nodig voor het kopiëren van BLOB's tussen opslag-accounts (uitgelegd later in het onderwerp.)
1. Maak een locator origin voor de activa die is gemaakt door de codering taak. 

Vervolgens gaat u naar het verwerken van de overname:

1. Stel een Media Services-account in "Data Center B".
1. Maak een lege doel-activa in de doellijst Media Services-account.
1. Maak een locator schrijven SA's voor de doeltoepassing lege activa schrijven toegang heeft tot de container in de doellijst opslag-account dat is gekoppeld aan de doel-activa.
1. Azure opslag SDK gebruiken om te kopiëren van BLOB's (activa-bestanden) tussen de bron opslag-account in "Data Center A" en doel opslag-account in "Data Center B" (deze opslag-accounts zijn gekoppeld aan de activa van belang.)
1. Koppel BLOB's (activa-bestanden) die aan de container doel blob met de doel-activa zijn gekopieerd. 
1. Een locator origin voor de activa in "Data Center B" maken en geef de locator Id die is gegenereerd voor de activa in "Data Center A". 
1. Hiermee geeft u de streaming URL's waar de relatieve paden van de URL's zijn hetzelfde (alleen het grondtal URL's verschillende zijn). 
 
Klik om te verwerken eventuele bijvoorbeeld, kunt u een CDN boven aan deze Locator origin maken. 

De volgende punten zijn van toepassing:

- De huidige versie van Media Services SDK biedt geen ondersteuning voor het maken van een locator met een opgegeven locator-id. Als u wilt bereiken deze taak, gebruiken we Media Services REST API.
- De huidige versie van Media Services SDK biedt geen ondersteuning voor via programmacode genereren IAssetFile informatie die activa met bestanden die u actief wilt koppelen. Als u wilt bereiken deze taak, gebruiken we de CreateFileInfos Media Services REST API. 
- Opslag versleuteld activa (AssetCreationOptions.StorageEncrypted) worden niet ondersteund voor replicatie (omdat de sleutel verschillend voor beide Media Services-accounts zijn). 
- Als u profiteren van dynamische verpakking wilt, moet u eerst ten minste één op aanvraag Streaming gereserveerde eenheden ophalen. Zie [Dynamisch verpakking activa](media-services-dynamic-packaging-overview.md)voor meer informatie.
 

>[AZURE.NOTE]Kunt u het Media-Services [Distributeur hulpmiddel](http://replicator.codeplex.com/) gebruiken als alternatief voor het implementeren van een failover streaming scenario handmatig. Dit hulpmiddel kunt u activa repliceren in twee Media Services-accounts.

##<a name="prerequisites"></a>Vereisten voor
 
- Twee Media Services-accounts in een nieuw of bestaand Azure abonnement. Lees [hoe u een Media Services-Account maken](media-services-portal-create-account.md).
- Besturingssystemen: Windows 7, Windows 2008 R2 of Windows 8.
- .NET framework 4.5 of .NET Framework 4.
- Visual Studio 2010 SP1 of hoger (Professional, Premium, Ultimate of Express).
 
##<a name="set-up-your-project"></a>Uw project instellen

In deze sectie wordt u maakt en een C#-Console-toepassing-project instellen.

1. Gebruik Visual Studio om een nieuwe oplossing met het project C#-Console-toepassing te maken. Voer HandleRedundancyForOnDemandStreaming voor de naam en klik vervolgens op OK.
1. Maak de map SupportFiles op hetzelfde niveau als het projectbestand HandleRedundancyForOnDemandStreaming.csproj. Maak de mappen OutputFiles en MP4Files onder de map SupportFiles. Kopieer een .mp4-bestand naar de map MP4Files (in dit voorbeeld die de BigBuckBunny.mp4-bestand wordt gebruikt). 
1. Gebruik **Nuget** om toe te voegen verwijzingen naar Media Services gerelateerde dll-bestanden. In Visual Studio hoofdmenu-Selecteer Extra > bibliotheek Package Manager -> Package Manager-Console. Typ in het console-installatiepakket enter windowsazure.mediaservices en druk op.
1. Andere verwijzingen die vereist voor dit project zijn toevoegen: System.Configuration, System.Runtime.Serialization en System.Web.
1. Vervangen met instructies die zijn toegevoegd aan het bestand Programs.cs al dan niet standaard met de volgende resultaten:
...
1. De sectie appSettings naar het bestand .config toevoegen en bijwerken van de waarden op basis van uw sleutel Media Services en opslag en waarden van de naam. 
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Voeg code redundantie voor het op aanvraag streaming verwerkt.



1. De volgende velden in de klas niveau toevoegen aan de klas programma.
...
1. De definitie van standaard Hoofdgegeven methode vervangen door de volgende:
...
1. Methodedefinities die worden aangeroepen vanuit Hoofdgegeven worden hieronder gedefinieerd.
...
U kunt nu een manager verkeer om route aanvragen tussen de twee datacenters en kan dus failover in het geval van eventuele bijvoorbeeld gebruiken.
