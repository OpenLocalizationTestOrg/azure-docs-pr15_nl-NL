<properties 
    pageTitle="ContentKeys maken met REST | Microsoft Azure" 
...
#<a name="create-contentkeys-with-rest"></a>ContentKeys met REST maken
...
Media-Services kunt u nieuwe maken en geven van versleutelde activa. Een **ContentKey** biedt beveiligde toegang tot uw **activa**s. 

Wanneer u een nieuw activum (bijvoorbeeld voordat u [bestanden uploaden](media-services-rest-upload-files.md)) maakt, kunt u de volgende versleutelingsopties: **StorageEncrypted**, **CommonEncryptionProtected**of **EnvelopeEncryptionProtected**. 

Als u activa wilt op uw klanten overbrengen, kunt u [configureren voor activa dynamisch worden versleuteld](media-services-rest-configure-asset-delivery-policy.md) met een van de volgende twee coderingen: **DynamicEnvelopeEncryption** of **DynamicCommonEncryption**.
...
1. Genereer willekeurig een 16-byte AES-sleutel (voor gemene en envelop versleuteling) of een 32-byte AES-sleutel (voor opslag-codering). 
...
3.  Uw inhoud sleutel met de openbare sleutel van de X.509-certificaat versleutelen. 
    ...
    Media Services .NET SDK gebruikt RSA OAEP bij het uitvoeren van de versleuteling.  Hier ziet u een voorbeeld in de [functie EncryptSymmetricKeyData](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs).
4.  Maak een controlesomwaarde (gebaseerd op de algoritme van de belangrijkste controlesom PlayReady AES) berekend met behulp van de sleutel-id en de inhoud sleutel. Zie voor meer informatie de sectie "PlayReady AES-toets controlesom algoritme" van het document PlayReady koptekst Object bevindt [hier](http://www.microsoft.com/playready/documents/).

    De volgende .NET-functie berekent de controlesom met het gedeelte GUID van de sleutel-id en de Schakel het selectievakje inhoud sleutel.
...
6. De entiteit **ContentKey** koppelen aan uw **activa** entiteit tot en met de bewerking $links.

Houd er rekening mee dat voorbeelden die een AES-sleutel genereren, versleutelt u de toets en de controlesom berekenen van dit onderwerp zijn weggelaten. Alleen de voorbeelden zien hoe u ermee Media Services worden gegeven.
...
Een van de waarden die u moet in te stellen wanneer de inhoud maken sleutel is het type. Kies een van de volgende waarden.
