<properties
    pageTitle="Azure Active Directory-rapportage controle-API voorbeelden | Microsoft Azure"
...
# <a name="azure-active-directory-reporting-audit-api-samples"></a>Azure Active Directory rapportage controle API-voorbeelden
...
Het bereik van dit onderwerp is bedoeld als u met code van de steekproef voor de **controle-API**.
...
App Auth werkt niet als uw tenant in de Europese Unie regio is. Gebruik gebruikersverificatie voor toegang krijgen tot de API controle als een tijdelijke oplossing totdat we het probleem verhelpt. 
...
### <a name="executing-the-powershell-script"></a>Het PowerShell-script
...
Het script retourneert uitvoer van het auditlograpport in JSON-indeling. Er worden ook gemaakt een `audit.json` bestand met hetzelfde resultaat op. U kunt experimenteren met het aanpassen van het script om terug te keren gegevens uit andere rapporten en commentaar de uitvoerindelingen die u niet nodig hebt.


## <a name="bash-script"></a>We vaker doen script
...
- Wilt u aanpassen in de voorbeelden in dit onderwerp? Bekijk de [Azure Active Directory controle API verwijzing](active-directory-reporting-api-audit-reference.md). 
